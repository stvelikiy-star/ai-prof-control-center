...
echo "=================================================="
echo " AK BERMET — RESTORE + HOST DISCOVERY V2"
echo "=================================================="
...
if [[ -z "$BACKUP" ]]; then
  ...
  echo "BACKUP=$BACKUP"
  echo "BACKUP_ROLES_SHA256=$(sha256sum "$BACKUP/roles.sql" | awk '{print $1}')"
  echo "BACKUP_SCHEMA_SHA256=$(sha256sum "$BACKUP/schema.sql" | awk '{print $1}')"
  echo "BACKUP_DATA_SHA256=$(sha256sum "$BACKUP/data.sql" | awk '{print $1}')"

  install -m 0644 "$BACKUP/roles.sql" "$STAGE/roles.sql"
  install -m 0644 "$BACKUP/schema.sql" "$STAGE/schema.sql"
  install -m 0644 "$BACKUP/data.sql" "$STAGE/data.sql"

  test "$(sha256sum "$BACKUP/roles.sql" | awk '{print $1}')" = "$(sha256sum "$STAGE/roles.sql" | awk '{print $1}')"
  test "$(sha256sum "$BACKUP/schema.sql" | awk '{print $1}')" = "$(sha256sum "$STAGE/schema.sql" | awk '{print $1}')"
  test "$(sha256sum "$BACKUP/data.sql" | awk '{print $1}')" = "$(sha256sum "$STAGE/data.sql" | awk '{print $1}')"
  ...
  if [[ "$READY" -ne 1 ]]; then
    ...
    docker exec -u postgres "$CONTAINER" \
      psql --variable ON_ERROR_STOP=1 --dbname restore_smoke --file /backup/roles.sql \
      >"$LOG" 2>&1
    RC1=$?
...
echo "AK_BERMET_BIG_GATE_2_V2=DONE"
