...
gateway_spec = importlib.util.spec_from_file_location("ai_prof_github_task_gateway", GATEWAY_PATH)
...
def contract(title: str = "Gateway test") -> dict:
    return {
        ...
        "forbidden_actions": [
            "commit", "push", "merge", "deployment", "secrets",
            "destructive-operations",
        ],
        "owner_approval_gates": ["production activation requires separate owner approval"],
        "acceptance_criteria": ["tests pass", "existing safety remains fail-closed"],
    }


def issue(number: int = 3, *, login: str = "stvelikiy-star", task: dict | None = None) -> dict:
...
class GitHubTaskGatewayTests(unittest.TestCase):
    def test_v1_is_bound_to_private_control_repository_and_owner(self):
        self.assertEqual(gateway.REPOSITORY, "stvelikiy-star/ai-prof-control-center")
    ...
    def test_valid_contract_is_strictly_parsed(self):
        ...
        self.assertTrue(gateway.REQUIRED_FORBIDDEN.issubset(set(parsed["forbidden_actions"])))
    ...
    def test_authorization_requires_exact_owner_and_rejects_prs(self):
        ...
        self.assertFalse(gateway.authorized_issue(issue(login="someone-else")))
        pr = issue()
    ...
    def test_submit_contract_calls_validator_without_shell(self):
        result = subprocess.CompletedProcess(
            [], 0, json.dumps({"task_id": "AI_PROF_CONTROL_CENTER_20260812T100000Z_ABCDEF", "queue": "pending"}), ""
        )
        with mock.patch.object(gateway.subprocess, "run", return_value=result) as run:
            created = gateway.submit_contract(44, gateway.parse_contract(issue(44)))
    ...
    def test_same_issue_is_imported_only_once(self):
        state: dict[str, dict] = {}
        created = {"task_id": "AI_PROF_CONTROL_CENTER_20260812T100000Z_ABCDEF", "queue": "pending"}
        with mock.patch.object(gateway, "submit_contract", return_value=created) as submit, \
        ...
        with mock.patch.object(gateway, "submit_contract") as submit_again, \
             mock.patch.object(gateway, "task_public_state", return_value=("queued", "")), \
    ...
    def test_unauthorized_author_is_rejected_without_enqueue(self):
        ...
        with mock.patch.object(gateway, "submit_contract") as submit, \
             mock.patch.object(gateway, "post_comment") as comment:
            self.assertTrue(gateway.process_issue(issue(88, login="intruder"), state))
    ...
    def test_comments_are_secret_redacted_before_github_call(self):
        with mock.patch.object(gateway, "run_gh") as run:
            gateway.post_comment(3, "password=SUPERSECRET token=ANOTHERTOKEN normal")
        argv = run.call_args.args[0]
    ...
    def test_gateway_state_is_atomic_and_mode_0600(self):
        with tempfile.TemporaryDirectory(prefix="ai-prof-gateway-state-") as tmp:
            path = Path(tmp) / "nested" / "state.json"
            payload = {"version": 1, "issues": {"3": {"status": "rejected"}}}
    ...
    def test_gh_runner_uses_argv_not_shell_and_disables_prompts(self):
        ...
        with mock.patch.object(gateway.shutil, "which", return_value="/usr/bin/gh"), \
             mock.patch.object(gateway.subprocess, "run", return_value=completed) as run:
            gateway.run_gh(["api", "repos/stvelikiy-star/ai-prof-control-center"])
        argv = run.call_args.args[0]
        kwargs = run.call_args.kwargs
    ...
    def test_systemd_gateway_is_hardened_and_has_no_secret_environment(self):
        unit = (ROOT / "systemd" / "ai-prof-github-task-gateway.service").read_text(encoding="utf-8")
        self.assertIn("github_task_gateway.py --poll-seconds 60", unit)
        self.assertIn("NoNewPrivileges=true", unit)
        self.assertIn("ProtectSystem=strict", unit)
        self.assertIn("ProtectHome=read-only", unit)
        self.assertIn("ReadWritePaths=/home/agent/.local/state/ai-prof-control-center", unit)
